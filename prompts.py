...
sales_team_prompt = """         
Give response in well-structured format with clear insights and key points, I will display on my Streamlit frontend, don't write ``` or md or markdown in beginning of the response, just write the response, I will convert the md text by myself.
Give point-wise, with space between each point, and use **bold** for headings and subheadings.
Strictly avoid the usage of words like etc, others, many more as I need full details in the report, no names or numbers should be missed in the report.
You will be provided with the following monthly work reports:
{
monthly_work_reports
}
...
**Dealer Visits and Discussions**
* Total number of dealerships visited (e.g., "Total Dealerships Visited: 25"), including a sample list of notable dealers (e.g., "Notable Dealers: Dealer A, Dealer B, Dealer C").
* Key topics discussed with dealers (e.g., "Key Discussion Topics: CF cases, insurance, warranties, car scope activities").
* Specific CF cases collected from dealers, with counts and dealer names (e.g., "CF Cases Collected: 10 cases (3 from Dealer A, 2 from Dealer B, 5 from Dealer C)").
...
**New Dealer Onboarding**
* Number of new dealers onboarded (e.g., "New Dealers Onboarded: 5"), with a sample list of names (e.g., "New Dealers: Dealer X, Dealer Y, Dealer Z").
* Additional onboarding actions taken (e.g., "Onboarding Actions: Created WhatsApp groups, scheduled initial training sessions").
...
sales_team_daily_prompt = """
Give response in well-structured format with clear insights and key points for a **daily** report, I will display on my Streamlit frontend, don't write ``` or md or markdown in beginning of the response, just write the response, I will convert the md text by myself.
Give point-wise, with space between each point, and use **bold** for headings and subheadings.
Strictly avoid the usage of words like etc, others, many more as I need full details in the report, no names or numbers should be missed in the report.
You will be provided with the following daily work reports:
{
daily_work_reports
}

Analyze the reports and create a concise summary for the specific day in the following format:

**Daily Dealer Visits**
* Dealerships visited today (e.g., "Dealerships Visited: Dealer A, Dealer B").
* Key topics discussed (e.g., "Topics Discussed: CF cases, insurance").

**Daily CF Case Updates**
* CF cases received today (e.g., "CF Cases Received: 3 (2 from Dealer A, 1 from Dealer B)").
* CF cases approved or disbursed (e.g., "Approved: 1, Disbursed: 0").

**Other Activities**
* Additional tasks or notes (e.g., "Other: Scheduled onboarding for Dealer C").

Ensure all data is extracted from the reports. Include specific dealer names, numbers, and details for the day. Use bullet points under each section as shown.
"""

PROMPTS = {
    "Product Development EDR": 
        "Analyze the monthly work reports from a member of Product Development EDR team. Provide a concise summary with the following sections:\n"
        "Focus on writing the end result of completed tasks, ongoing work, and pending items, rather than writing intermediate achievements; if the task is not completed, then go for intermediate issues\n"
        "- **Completed tasks:** List specific features, modules, or components completed, including successful tests, deployments, or integrations.\n"
        "- **Ongoing work:** Describe the status of major projects or sprints, noting ongoing tasks like coding, testing, or debugging.\n"
        "- **Pending items:** Highlight tasks behind schedule, bugs needing fixes, or backlog features.\n"
        "- **Key insights:** Identify patterns in challenges (e.g., resource constraints) or successes (e.g., meeting deadlines), and note any innovations or process improvements.\n"
...
DAILY_PROMPTS = {
    "Product Development EDR": 
        "Analyze the daily work reports from a member of Product Development EDR team. Provide a concise summary with the following sections:\n"
        "- **Completed Tasks:** List features, modules, or components completed today, including tests or integrations.\n"
        "- **Ongoing Tasks:** Describe tasks in progress, such as coding or debugging.\n"
        "- **Issues or Blockers:** Highlight any bugs, delays, or resource constraints faced today.\n"
        "Use bullet points for each section.",

    "HR and Admin": 
        "Analyze the daily work reports from a member of HR and Admin team. Provide a concise summary with the following sections:\n"
        "- **Completed Tasks:** List HR or admin tasks completed today, like onboarding or payroll tasks.\n"
        "- **Ongoing Tasks:** Describe ongoing processes, such as recruitment or training.\n"
        "- **Pending Tasks:** Highlight any urgent tasks or issues pending resolution.\n"
        "Use bullet points for each section.",

    "Management EDR": 
        "Analyze the daily work reports from a member of Management EDR team. Provide a concise summary with the following sections:\n"
        "- **Completed Tasks:** List decisions made or approvals given today.\n"
        "- **Ongoing Tasks:** Describe ongoing activities, like project reviews or strategy meetings.\n"
        "- **Pending Tasks:** Highlight pending decisions or meetings scheduled.\n"
        "Use bullet points for each section.",

    "Marketing EDR": 
        "Analyze the daily work reports from a member of the Marketing EDR team. Provide a concise summary with the following sections:\n"
        "- **Completed Tasks:** List content published, campaigns launched, or events held today.\n"
        "- **Ongoing Tasks:** Describe ongoing activities, like social media posts or market research.\n"
        "- **Pending Tasks:** Highlight delayed tasks or upcoming deadlines.\n"
        "Use bullet points for each section.",

    "Accounts EDR": 
        "Analyze the daily work reports from a member of Accounts EDR team. Provide a concise summary with the following sections:\n"
        "- **Completed Tasks:** List financial tasks completed today, like invoices processed or payments received.\n"
        "- **Ongoing Tasks:** Describe ongoing activities, like expense tracking or tax preparation.\n"
        "- **Pending Tasks:** Highlight overdue payments or unresolved issues.\n"
        "Use bullet points for each section.",

    "BLR EDR": sales_team_daily_prompt, 
    "DEL NCR EDR": sales_team_daily_prompt,
    "Himachal EDR": sales_team_daily_prompt,
    "HYD EDR": sales_team_daily_prompt,
    "Kolkata EDR": sales_team_daily_prompt,
    "MUM EDR": sales_team_daily_prompt,
    "PUNE EDR": sales_team_daily_prompt, 
    "Punjab and Chd EDR": sales_team_daily_prompt,
    "RJ EDR": sales_team_daily_prompt
}

def get_prompt(team: str) -> str:
    return PROMPTS.get(team, "Summarize the work report with clear insights and key points.")

def get_daily_prompt(team: str) -> str:
    return DAILY_PROMPTS.get(team, "Summarize the daily work report with clear insights and key points.")